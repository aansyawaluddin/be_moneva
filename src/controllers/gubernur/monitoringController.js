...
const INCLUDE_INTEGRITAS = {
    detailGaspoll: true, detailSpbe: true,
    detailBudayaKerja: true, detailBantuanKeuangan: true,
};
                        ...
                        include: {
                            ...
                            dataRealisasi: {
                                ...
                                include: {
                                    ...
                                    ...INCLUDE_SEHAT, ...INCLUDE_SEJAHTERA, ...INCLUDE_LANCAR, ...INCLUDE_HARMONI, ...INCLUDE_INTEGRITAS, ...INCLUDE_MENYALA,
                                }
                            }
                        }
                    }
            ...
            const monitoringData = allData.map(program => ({
                ...
                "Daftar Sub Program": program.subProgram.map(sub => {
                    ...
                    sub.dataRealisasi.forEach(upload => {
                        ...
                        const harmoniArrays = [upload.detailWirausaha, upload.detailBerbudaya, upload.detailInvestasi, upload.detailBermitra, upload.detailHarmoniCareer, upload.detailEkonomiKreatif, upload.detailLestari, upload.detailProduktif, upload.detailWisata, upload.detailOlahraga];
                        ...
                        [...sehatArrays, ...sejahteraArrays, ...lancarArrays, ...harmoniArrays, ...integritasArrays, ...menyalaArrays].forEach(hitungAdaptif);
                        ...
                        [...sehatArrays, ...sejahteraArrays, ...lancarArrays, ...harmoniArrays, ...integritasArrays, ...menyalaArrays].forEach(arr => { arr?.forEach(item => { const n = item.realisasiAnggaran ? Number(item.realisasiAnggaran.toString()) : 0; uangLaporan += isNaN(n) ? 0 : n; }); });
                    ...
                    return {
                        ...
                        "Target Fisik": Math.round(targetFisik * 100000) / 100000, "Realisasi Fisik": Math.round(totalDisetujuiFisik * 100000) / 100000,
        ...
        try {
            ...
            const mapSejahtera = (raw, withKab = false) => raw.map(item => {
                ...
                return {
                    ...
                    ...(withKab && { "Kabupaten / Kota": item.kabupatenKota || '-' }),
                    "Satuan": item.satuan || '-',
                    "Target Kinerja": item.targetKinerja,
                    "Target Anggaran": formatRp(Number(item.targetAnggaran)),
                    "Realisasi Kinerja": item.realisasiKinerja,
            ...
            if (nameLower.includes('bosda') || nameLower.includes('operasional')) {
            ...
            } else if ((nameLower.includes('digital') && !nameLower.includes('investasi') && !nameLower.includes('berinvestasi')) || (nameLower.includes('sarana') && !nameLower.includes('olahraga') && !nameLower.includes('stadion') && !nameLower.includes('sport'))) {
                type = "digital";
                ...
                totalFisik = raw.length;
                data = raw.map(item => ({ id: item.id, "Nama Penerima": item.namaPenerima, "NIK": item.nik || '-', "NIM": item.nim || '-', "Program Studi": item.programStudi || '-', "Institusi Tujuan": item.institusiTujuan, "Kabupaten / Kota": item.kabupaten, "Jalur": item.jalur || '-', "Nominal Bantuan": formatRp(getNominal(item.nominal)), "Kontak Penerima": item.kontakPenerima || '-' }));
                ...
                type = "wirausaha";
                const raw = await prisma.realisasiWirausaha.findMany({ where: baseWhere, include: { header: true }, orderBy: { header: { tanggalVerifikasi: 'asc' } } });
                ...
                type = "berbudaya";
                const raw = await prisma.realisasiBerbudaya.findMany({ where: baseWhere, include: { header: true }, orderBy: { header: { tanggalVerifikasi: 'asc' } } });
                data = raw.map(item => { hitungAdaptifFisik(item.realisasiKinerja); return { id: item.id, "Perangkat Daerah": item.perangkatDaerah || '-', "Rincian Kegiatan": item.rincianKegiatan || '-', "Kabupaten / Kota": item.kabupatenKota || '-', "Satuan": item.satuan || '-', "Target Kinerja": item.targetKinerja, "Target Anggaran": formatRp(Number(item.targetAnggaran)), "Realisasi Kinerja": item.realisasiKinerja, "Realisasi Anggaran": formatRp(getNominal(item.realisasiAnggaran)), "Capaian Kinerja (%)": item.capaianKinerja || '-', "Capaian Anggaran (%)": item.capaianAnggaran || '-' }; });
            } else if (nameLower.includes('investasi') || nameLower.includes('berinvestasi')) {
                type = "investasi";
                const raw = await prisma.realisasiInvestasi.findMany({ where: baseWhere, include: { header: true }, orderBy: { header: { tanggalVerifikasi: 'asc' } } });
                data = raw.map(item => { hitungAdaptifFisik(item.realisasiKinerja); return { id: item.id, "Perangkat Daerah": item.perangkatDaerah || '-', "Rincian Kegiatan": item.rincianKegiatan || '-', "Kabupaten / Kota": item.kabupatenKota || '-', "Satuan": item.satuan || '-', "Target Kinerja": item.targetKinerja, "Target Anggaran": formatRp(Number(item.targetAnggaran)), "Realisasi Kinerja": item.realisasiKinerja, "Realisasi Anggaran": formatRp(getNominal(item.realisasiAnggaran)), "Capaian Kinerja (%)": item.capaianKinerja || '-', "Capaian Anggaran (%)": item.capaianAnggaran || '-' }; });
            } else if (nameLower.includes('bermitra') || nameLower.includes('koperasi merah putih')) {
                type = "bermitra";
                const raw = await prisma.realisasiBermitra.findMany({ where: baseWhere, include: { header: true }, orderBy: { header: { tanggalVerifikasi: 'asc' } } });
                data = raw.map(item => { hitungAdaptifFisik(item.realisasiKinerja); return { id: item.id, "Perangkat Daerah": item.perangkatDaerah || '-', "Rincian Kegiatan": item.rincianKegiatan || '-', "Kabupaten / Kota": item.kabupatenKota || '-', "Satuan": item.satuan || '-', "Target Kinerja": item.targetKinerja, "Target Anggaran": formatRp(Number(item.targetAnggaran)), "Realisasi Kinerja": item.realisasiKinerja, "Realisasi Anggaran": formatRp(getNominal(item.realisasiAnggaran)), "Capaian Kinerja (%)": item.capaianKinerja || '-', "Capaian Anggaran (%)": item.capaianAnggaran || '-' }; });
            } else if ((nameLower.includes('carier center') || nameLower.includes('career center')) && nameLower.includes('berani')) {
                type = "harmoni-career";
                const raw = await prisma.realisasiHarmoniCareer.findMany({ where: baseWhere, include: { header: true }, orderBy: { header: { tanggalVerifikasi: 'asc' } } });
                data = raw.map(item => { hitungAdaptifFisik(item.realisasiKinerja); return { id: item.id, "Perangkat Daerah": item.perangkatDaerah || '-', "Rincian Kegiatan": item.rincianKegiatan || '-', "Kabupaten / Kota": item.kabupatenKota || '-', "Satuan": item.satuan || '-', "Target Kinerja": item.targetKinerja, "Target Anggaran": formatRp(Number(item.targetAnggaran)), "Realisasi Kinerja": item.realisasiKinerja, "Realisasi Anggaran": formatRp(getNominal(item.realisasiAnggaran)), "Capaian Kinerja (%)": item.capaianKinerja || '-', "Capaian Anggaran (%)": item.capaianAnggaran || '-' }; });
            } else if (nameLower.includes('ekonomi kreatif') || nameLower.includes('sulteng creative')) {
                type = "ekonomi-kreatif";
                const raw = await prisma.realisasiEkonomiKreatif.findMany({ where: baseWhere, include: { header: true }, orderBy: { header: { tanggalVerifikasi: 'asc' } } });
                ...
                type = "lestari";
                const raw = await prisma.realisasiLestari.findMany({ where: baseWhere, include: { header: true }, orderBy: { header: { tanggalVerifikasi: 'asc' } } });
                data = raw.map(item => { hitungAdaptifFisik(item.realisasiKinerja); return { id: item.id, "Perangkat Daerah": item.perangkatDaerah || '-', "Rincian Kegiatan": item.rincianKegiatan || '-', "Kabupaten / Kota": item.kabupatenKota || '-', "Satuan": item.satuan || '-', "Target Kinerja": item.targetKinerja, "Target Anggaran": formatRp(Number(item.targetAnggaran)), "Realisasi Kinerja": item.realisasiKinerja, "Realisasi Anggaran": formatRp(getNominal(item.realisasiAnggaran)), "Capaian Kinerja (%)": item.capaianKinerja || '-', "Capaian Anggaran (%)": item.capaianAnggaran || '-' }; });
                ...
                type = "produktif";
                const raw = await prisma.realisasiProduktif.findMany({ where: baseWhere, include: { header: true }, orderBy: { header: { tanggalVerifikasi: 'asc' } } });
                data = raw.map(item => { hitungAdaptifFisik(item.realisasiKinerja); return { id: item.id, "Perangkat Daerah": item.perangkatDaerah || '-', "Rincian Kegiatan": item.rincianKegiatan || '-', "Kabupaten / Kota": item.kabupatenKota || '-', "Satuan": item.satuan || '-', "Target Kinerja": item.targetKinerja, "Target Anggaran": formatRp(Number(item.targetAnggaran)), "Realisasi Kinerja": item.realisasiKinerja, "Realisasi Anggaran": formatRp(getNominal(item.realisasiAnggaran)), "Capaian Kinerja (%)": item.capaianKinerja || '-', "Capaian Anggaran (%)": item.capaianAnggaran || '-' }; });
            } else if (nameLower.includes('wisata')) {
                type = "wisata";
                const raw = await prisma.realisasiWisata.findMany({ where: baseWhere, include: { header: true }, orderBy: { header: { tanggalVerifikasi: 'asc' } } });
                data = raw.map(item => { hitungAdaptifFisik(item.realisasiKinerja); return { id: item.id, "Perangkat Daerah": item.perangkatDaerah || '-', "Rincian Kegiatan": item.rincianKegiatan || '-', "Kabupaten / Kota": item.kabupatenKota || '-', "Satuan": item.satuan || '-', "Target Kinerja": item.targetKinerja, "Target Anggaran": formatRp(Number(item.targetAnggaran)), "Realisasi Kinerja": item.realisasiKinerja, "Realisasi Anggaran": formatRp(getNominal(item.realisasiAnggaran)), "Capaian Kinerja (%)": item.capaianKinerja || '-', "Capaian Anggaran (%)": item.capaianAnggaran || '-' }; });
            } else if (nameLower.includes('olahraga') || nameLower.includes('stadion') || nameLower.includes('sport center')) {
                type = "olahraga";
                const raw = await prisma.realisasiOlahraga.findMany({ where: baseWhere, include: { header: true }, orderBy: { header: { tanggalVerifikasi: 'asc' } } });
                data = raw.map(item => { hitungAdaptifFisik(item.realisasiKinerja); return { id: item.id, "Perangkat Daerah": item.perangkatDaerah || '-', "Satuan": item.satuan || '-', "Target Kinerja": item.targetKinerja, "Target Anggaran": formatRp(Number(item.targetAnggaran)), "Realisasi Kinerja": item.realisasiKinerja, "Realisasi Anggaran": formatRp(getNominal(item.realisasiAnggaran)), "Capaian Kinerja (%)": item.capaianKinerja || '-', "Capaian Anggaran (%)": item.capaianAnggaran || '-' }; });
                ...
                type = "panada";
                const raw = await prisma.realisasiPanada.findMany({ where: baseWhere, include: { header: true }, orderBy: { header: { tanggalVerifikasi: 'asc' } } });
                data = mapSejahtera(raw);
            } else if (nameLower.includes('uep') || nameLower.includes('usaha ekonomi produktif') || nameLower.includes('graduasi')) {
                type = "uep";
                const raw = await prisma.realisasiUep.findMany({ where: baseWhere, include: { header: true }, orderBy: { header: { tanggalVerifikasi: 'asc' } } });
                data = mapSejahtera(raw);
            } else if (nameLower.includes('rutilahu')) {
                type = "rutilahu";
                const raw = await prisma.realisasiRutilahu.findMany({ where: baseWhere, include: { header: true }, orderBy: { header: { tanggalVerifikasi: 'asc' } } });
                data = mapSejahtera(raw, true);
            } else if (nameLower.includes('umkm') || nameLower.includes('kewirausahaan')) {
                type = "umkm";
                const raw = await prisma.realisasiUmkm.findMany({ where: baseWhere, include: { header: true }, orderBy: { header: { tanggalVerifikasi: 'asc' } } });
                data = mapSejahtera(raw, true);
            } else if (nameLower.includes('mbg') || nameLower.includes('makan bergizi')) {
                type = "mbg";
                const raw = await prisma.realisasiMbg.findMany({ where: baseWhere, include: { header: true }, orderBy: { header: { tanggalVerifikasi: 'asc' } } });
                data = mapSejahtera(raw);

                // ===== BERANI LANCAR =====
            } else if (nameLower.includes('air bersih') || (nameLower.includes('jaminan') && nameLower.includes('air'))) {
                type = "air-bersih";
                const raw = await prisma.realisasiAirBersih.findMany({ where: baseWhere, include: { header: true }, orderBy: { header: { tanggalVerifikasi: 'asc' } } });
                data = raw.map(item => { hitungAdaptifFisik(item.realisasiKinerja); return { id: item.id, "Uraian": item.uraian || '-', "Kabupaten / Kota": item.kabupatenKota || '-', "Satuan": item.satuan || '-', "Target Kinerja": item.targetKinerja, "Target Anggaran": formatRp(Number(item.targetAnggaran)), "Realisasi Kinerja": item.realisasiKinerja, "Realisasi (%) Kinerja": item.realisasiPersenKinerja || '-', "Realisasi Anggaran": formatRp(getNominal(item.realisasiAnggaran)) }; });
            } else if (nameLower.includes('drainase')) {
                type = "drainase";
                const raw = await prisma.realisasiDrainase.findMany({ where: baseWhere, include: { header: true }, orderBy: { header: { tanggalVerifikasi: 'asc' } } });
                data = raw.map(item => { hitungAdaptifFisik(item.realisasiKinerja); return { id: item.id, "Uraian": item.uraian || '-', "Kabupaten / Kota": item.kabupatenKota || '-', "Satuan": item.satuan || '-', "Target Kinerja": item.targetKinerja, "Target Anggaran": formatRp(Number(item.targetAnggaran)), "Realisasi Kinerja": item.realisasiKinerja, "Realisasi (%) Kinerja": item.realisasiPersenKinerja || '-', "Realisasi Anggaran": formatRp(getNominal(item.realisasiAnggaran)) }; });
            } else if (nameLower.includes('multi year contract') || nameLower.includes('myc')) {
                type = "myc";
                const raw = await prisma.realisasiMyc.findMany({ where: baseWhere, include: { header: true }, orderBy: { header: { tanggalVerifikasi: 'asc' } } });
                data = raw.map(item => { hitungAdaptifFisik(item.realisasiKinerja); return { id: item.id, "Sasaran / Program": item.sasaranProgram || '-', "Kabupaten / Kota": item.kabupatenKota || '-', "Satuan": item.satuan || '-', "Target Kinerja": item.targetKinerja, "Target Anggaran": formatRp(Number(item.targetAnggaran)), "Realisasi Kinerja": item.realisasiKinerja, "Realisasi Anggaran": formatRp(getNominal(item.realisasiAnggaran)) }; });
            } else if (nameLower.includes('agropolitan') || nameLower.includes('tongkol') || nameLower.includes('cakalang') || nameLower.includes('hilirisasi')) {
                type = "agropolitan";
                const raw = await prisma.realisasiAgropolitan.findMany({ where: baseWhere, include: { header: true }, orderBy: { header: { tanggalVerifikasi: 'asc' } } });
                data = raw.map(item => { hitungAdaptifFisik(item.realisasiKinerja); return { id: item.id, "Perangkat Daerah": item.perangkatDaerah || '-', "Satuan": item.satuan || '-', "Target Kinerja": item.targetKinerja, "Target Anggaran": formatRp(Number(item.targetAnggaran)), "Realisasi Kinerja": item.realisasiKinerja, "Realisasi Anggaran": formatRp(getNominal(item.realisasiAnggaran)), "Capaian Kinerja (%)": item.capaianKinerja || '-', "Capaian Anggaran (%)": item.capaianAnggaran || '-' }; });
            } else if (nameLower.includes('1000 kilometer') || nameLower.includes('jalan desa')) {
                type = "jalan-desa";
                const raw = await prisma.realisasiJalanDesa.findMany({ where: baseWhere, include: { header: true }, orderBy: { header: { tanggalVerifikasi: 'asc' } } });
                data = raw.map(item => { hitungAdaptifFisik(item.realisasiKinerja); return { id: item.id, "Perangkat Daerah": item.perangkatDaerah || '-', "Sasaran / Program": item.sasaranProgram || '-', "Kabupaten / Kota": item.kabupatenKota || '-', "Satuan": item.satuan || '-', "Target Kinerja": item.targetKinerja, "Target Anggaran": formatRp(Number(item.targetAnggaran)), "Realisasi Kinerja": item.realisasiKinerja, "Realisasi Anggaran": formatRp(getNominal(item.realisasiAnggaran)) }; });
            } else if (nameLower.includes('banggai kepulauan')) {
                type = "konektivitas-banggai";
                const raw = await prisma.realisasiKonektivitasBanggai.findMany({ where: baseWhere, include: { header: true }, orderBy: { header: { tanggalVerifikasi: 'asc' } } });
                data = raw.map(item => { hitungAdaptifFisik(item.realisasiKinerja); return { id: item.id, "Sasaran / Program": item.sasaranProgram || '-', "Kabupaten / Kota": item.kabupatenKota || '-', "Satuan": item.satuan || '-', "Target Kinerja": item.targetKinerja, "Target Anggaran": formatRp(Number(item.targetAnggaran)), "Realisasi Kinerja": item.realisasiKinerja, "Realisasi Anggaran": formatRp(getNominal(item.realisasiAnggaran)) }; });
                ...
                type = "konektivitas-tambu";
                const raw = await prisma.realisasiKonektivitasTambu.findMany({ where: baseWhere, include: { header: true }, orderBy: { header: { tanggalVerifikasi: 'asc' } } });
                data = raw.map(item => { hitungAdaptifFisik(item.realisasiKinerja); return { id: item.id, "Sasaran / Program": item.sasaranProgram || '-', "Kabupaten / Kota": item.kabupatenKota || '-', "Satuan": item.satuan || '-', "Target Kinerja": item.targetKinerja, "Target Anggaran": formatRp(Number(item.targetAnggaran)), "Realisasi Kinerja": item.realisasiKinerja, "Realisasi Anggaran": formatRp(getNominal(item.realisasiAnggaran)) }; });
            ...
            res.json({ status: "success", program: subProgram.programKerja.namaProgram, programSlug: subProgram.programKerja.slug, subProgram: subProgram.namaSubProgram, subProgramSlug: subProgram.slug, tahun, type, totalData: Math.round(totalFisik * 100000) / 100000, data });
