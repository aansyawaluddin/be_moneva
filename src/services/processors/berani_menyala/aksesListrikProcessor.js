...
const process = async (tx, headerId, filePath) => {
    ...
    if (dataExcel.length === 0) throw new Error("File Excel Akses Listrik kosong");
    ...
    if (detailData.length === 0) throw new Error("Tidak ada data valid di Excel Akses Listrik");
    await tx.realisasiAksesListrik.createMany({ data: detailData });
