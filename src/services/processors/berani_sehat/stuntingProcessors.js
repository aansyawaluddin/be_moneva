...
const process = async (tx, headerId, filePath) => {
    ...
    if (dataExcel.length === 0) throw new Error("Excel Pencegahan Stunting kosong");

    const detailData = dataExcel
        .filter(row => row['Program'] && String(row['Program']).trim() !== '')
        .map(row => ({
    ...
    if (detailData.length === 0) throw new Error("Tidak ada data valid di Excel Pencegahan Stunting");
    await tx.realisasiStunting.createMany({ data: detailData });
