...
const process = async (tx, headerId, filePath) => {
    ...
    if (dataExcel.length === 0) throw new Error("Excel RS Rujukan kosong");
    ...
    if (detailData.length === 0) throw new Error("Tidak ada data valid di Excel RS Rujukan");
    await tx.realisasiRsRujukan.createMany({ data: detailData });
