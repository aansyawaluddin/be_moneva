...
const process = async (tx, headerId, filePath) => {
    ...
    if (dataExcel.length === 0) throw new Error("Excel Kualitas RS kosong");
    ...
    if (detailData.length === 0) throw new Error("Tidak ada data valid di Excel Kualitas RS");
    await tx.realisasiKualitasRs.createMany({ data: detailData });
