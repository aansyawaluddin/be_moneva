...
const process = async (tx, headerId, filePath) => {
    ...
    if (dataExcel.length === 0) throw new Error("File Excel Beasiswa Miskin kosong");
    ...
    const detailData = dataExcel
        ...
        .map(row => {
            ...
            return {
                ...
                kabupaten: row['Kabupaten/Kota'] ? String(row['Kabupaten/Kota']).trim() : null,
                targetKinerja: Number(row['Target Kinerja '] ?? row['Target Kinerja']) || 0,
    ...
    if (detailData.length === 0) throw new Error("Tidak ada data valid di Excel Beasiswa Miskin");
